...
import eu.aniketos.wp2.components.trustworthiness.messaging.ReputationMetricsJms;
 ...
 *
 */
public class ReputationMetricsJmsImpl implements ReputationMetricsJms
{

	private static Logger logger = Logger.getLogger(ReputationMetricsJmsImpl.class);
	...
	public void receiveMetrics(){
		...
		/*try {
			...
			logger.debug("lookup rmi");
			Registry registry = LocateRegistry.getRegistry(config.getConfig().getString("memo_address"), 
					Integer.parseInt(config.getConfig().getString("memo_port")));

			metricStore = (MetricStore) registry.lookup(config.getConfig().getString("memo_server"));
			...
			for (String service : services) {
				...
				//1st query sla violations
				filter.setMemberId(service);
				filter.setMetricType(MetricType.SLA_VIOLATION_NUMBER.name());
				//considering last 60 secs only
				Date fromDate = new Date();
				fromDate.setTime(System.currentTimeMillis()-60000);
				filter.setFromDate(fromDate);
				
				Object message = metricStore.getLatestMetricValue(filter);
				String property = "sla";
				String subproperty = "sla-violation";
				...
				if (message instanceof MetricValue) {
					...
					logger.debug("metrics received for " + service);
					Map<String, String> metricMessage = new HashMap<String, String>();
					metricMessage.put("type", "metric");
					// TODO: metric type needs work to align with
					// properties/subproperties
					
					if (logger.isDebugEnabled()){
						logger.debug("metric type: " + metricValue.getMetricValueType());
						logger.debug("metric value: " + metricValue.getValue().toString());
						logger.debug("start time: " + metricValue.getStartTime());
						logger.debug("end time: " + metricValue.getEndTime());
					}
					
					metricMessage.put("property", property);
					metricMessage.put("subproperty", subproperty);
					metricMessage.put("service", service);
					metricMessage.put("metric", metricValue.getValue().toString());

					scoreUpdate.updateScore(metricMessage);

				} else if (message == null) {
					logger.debug("received null for " + service);
				}
